  ...
  private void setFormLoadedListener(@NotNull String accessToken,
    ...
    Platform.runLater(() -> getEngine().getLoadWorker().stateProperty().addListener((observable, oldState, newState) -> {
      if (newState == Worker.State.SUCCEEDED && getEngine().getLocation().contains("checkioTestForm.html")) {
        ...
        documentWithForm.getElementById("access-token").setAttribute("value", accessToken);
        documentWithForm.getElementById("task-id").setAttribute("value", taskId);
        documentWithForm.getElementById("interpreter").setAttribute("value", interpreter);
        documentWithForm.getElementById("code").setAttribute("value", code);

        HTMLFormElement formElement = (HTMLFormElement)documentWithForm.getElementById("test-form");
        formElement.submit();
